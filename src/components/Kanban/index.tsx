...
import multipleContainersCoordinateGetter from "@/components/Kanban/multipleContainersKeyboardCoordinates";
...
import DynamicFormDialog from "@/components/Kanban/KanbanColumnModal";
 ...
 // itemCount,
  ...
  const loadItems = async (): Promise<Record<string, string[]>> => {
    try {
      ...
      if (!tasksResponse.ok || !columnsResponse.ok)
        throw new Error("Failed to fetch tasks or columns");
  ...
  useEffect(() => {
    loadItems().then((data) => {
      setItems(data);
      setContainers(Object.keys(data) as UniqueIdentifier[]);
    });
  }, []);
          ...
          >
            ...
            {minimal ? undefined : (
              <DroppableContainer
                ...
                placeholder
              >
                <Box component={"button"}>+ Add column</Box>
              </DroppableContainer>
            )}
          </SortableContext>
        </Box>
  ...
  function handleAddColumn(data: Column) {
    //setOpen(true)
    const newColumn: Column = {
      id: data.id, // Generate a unique ID
      name: data.name, // Get a unique column name
      color: data.color, // Default color (you can change this)
      slug: data.name.toLowerCase().replace(/\s+/g, "-"), // Generate slug
    };
    //const newContainerId = getNextContainerId();

    unstable_batchedUpdates(() => {
      setContainers((prevContainers) => [...prevContainers, newColumn.name]); // Use column name
      setItems((prevItems) => ({
        ...prevItems,
        [newColumn.name]: [], // Initialize an empty array for tasks
      }));
    });
  }
...
function SortableItem({
  ...
  return (
    <KanbanItem
      ...
      color={columnColors[containerId]}
